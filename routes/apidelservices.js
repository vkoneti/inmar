/********************************************************************
Program: routes/apideleteservices.js
...
exports.delLocation = function (req, resp, next) {
    var lid = req.params.lid;

    var callDetails = "REQ(DELETE): api/v1/location/"+ lid;
...
exports.delDepartment = function (req, resp, next) {
    var lid = req.params.lid;
    var did = req.params.did;

    var callDetails = "REQ(DELETE): /api/v1/location/"+ lid +"/department/"+ did;
    Util.logReqBegin(callDetails);
...
exports.delCategory = function (req, resp, next) {
    var lid = req.params.lid;
    var did = req.params.did;
    var cid = req.params.cid;

    var callDetails = "REQ(DELETE): /api/v1/location/"+ lid +"/department/"+ did +"/category/"+ cid;
    Util.logReqBegin(callDetails);
...
exports.delSubCategory = function (req, resp, next) {
    var lid = req.params.lid;
    var did = req.params.did;
    var cid = req.params.cid;
    var sid = req.params.sid;

    var callDetails = "REQ(DELETE): /api/v1/location/"+ lid +"/department/"+ did +"/category/"+ cid +"/subcategory/"+ sid;
    Util.logReqBegin(callDetails);
        ...
        if(allSubCats.length > 0) {
            Subcategory.findOne({
                ...
                if (thisSubcategory == null) {
                    ...
                    // Remove/Delete this subcategory from DB
                    thisSubcategory.destroy().then( function(rowsDeleted) {
                        resp.status(200);
                        resp.send({ CallStatus: "GOOD", Data: rowsDeleted });
                        next();
                    });
                }
            });
